...
export class ClusteringService {
  ...
  private async findGroupByVector(
    ...
    try {
      ...
      if (
        rows.length > 0 &&
        rows[0].distance < ClusteringService.VECTOR_THRESHOLD
      ) {
        return rows[0].id;
      }
    } catch (err) {
  ...
  private async findGroupByTemporal(
    systemCode: string,
  ): Promise<string | null> {
    const group = await this.prisma.incidentGroup.findFirst({
      where: {
        ...
        lastSeenAt: {
          gt: new Date(
            Date.now() - ClusteringService.TEMPORAL_WINDOW_MIN * 60_000,
          ),
        },
      },
  ...
  private async createNewGroup(
    ...
    const embeddingData = embedding ? { centroidEmbedding: embedding } : {};
