...
export class IngestionService {
  ...
  async syncJira() {
    ...
    const timeWindows = await this.prisma.timeWindow.findMany({
      where: { isActive: true },
    });
  ...
  private classify(
    ...
    const systemMap: Array<[RegExp, SystemCode]> = [
      [
        /fatur|nota fiscal|nf|sefaz|boleto|cobran[cç]a|valor errado/i,
        SystemCode.GM_FIN,
      ],
      [
        /estoque|cd |wms|caminh[aã]o|motorista|frete|carga|expedi[cç]/i,
        SystemCode.GM_LOG,
      ],
      [
        /rede|internet|infraestrutura|impressora|datacenter|conectiv/i,
        SystemCode.GM_INFRA,
      ],
    ...
    let scoreS = 4;
    if (
      /parou|caiu|trav|parad|sem comunica|opera[cç][aã]o parada/i.test(lower)
    ) {
      scoreS = 9 + Math.random();
      keywordsFound.push('critical-severity');
    } else if (
      /erro|falha|n[aã]o funciona|n[aã]o consigo|fecha sozinho/i.test(lower)
    ) {
  ...
  private async groupFeedbacks() {
    ...
    const ungrouped = await this.prisma.processedFeedback.findMany({
      ...
      include: {
        rawFeedback: { select: { rawContent: true, receivedAt: true } },
      },
    });
    ...
    for (const pf of ungrouped) {
      const words = pf.rawFeedback.rawContent
        .toLowerCase()
        .split(/\s+/)
        .slice(0, 3)
        .join(' ');
    ...
    this.logger.log(
      `Agrupamento concluído: ${Object.keys(groups).length} grupos processados`,
    );
  }
