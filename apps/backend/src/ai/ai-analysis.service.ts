...
export class AIAnalysisService {
  ...
  constructor(
    ...
    this.featureEnabled = isFeatureEnabled(
      this.config.get<string>('FEATURE_AI_ENABLED'),
      false,
    );
    const apiKey = this.config.get<string>('ANTHROPIC_API_KEY');
    if (this.featureEnabled && apiKey) {
  ...
  async analyze(rawFeedback: RawFeedback): Promise<AIAnalysisResult> {
    if (!this.featureEnabled || !this.client) {
  ...
  private async callClaude(
    ...
    try {
      ...
      if (temperature === 0) {
        this.logger.warn(
          'Primeira tentativa falhou, retentando com temperature=0.1',
        );
